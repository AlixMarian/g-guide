...
const VisitaIglesia = () => {
      ...
      (initialResult, status) => {
        if (status === window.google.maps.DirectionsStatus.OK) {
          ...
          if (nearbyChurches.length < 7) {
            const remainingChurches = churches.filter(church => !nearbyChurches.some(nc => nc.id === church.id));
  ...
  const findChurchesAlongRoute = (churches, routePolyline, maxDistanceKm) => {
    return churches.filter(church => {
      const churchLocation = new window.google.maps.LatLng(parseFloat(church.latitude), parseFloat(church.longitude));

      // Check if the church is within maxDistanceKm of any point along the route
      const isNearby = routePolyline.some(routePoint => {
        const distance = window.google.maps.geometry.spherical.computeDistanceBetween(routePoint, churchLocation) / 1000; // Convert to km
  ...
  useEffect(() => {
    console.log('Current churches:', churches);
    console.log('Current position:', currentPosition);
    console.log('Custom icon:', customIcon);
  }, [churches, currentPosition, customIcon]);
  ...
  const uniqueDestinations = destinations.map(d => d.destination).filter((dest, index, self) => dest && self.findIndex(d => d.lat === dest.lat && d.lng === dest.lng) === index);
        ...
        >
          ...
          {churches.map(church => {
            ...
            if (!isNaN(lat) && !isNaN(lng)) {
              return <Marker key={church.id} position={{lat, lng}} icon={customIcon} onClick={() => handleMarkerClick(church)} />;
            }
            return null;
          })}
          ...
          <Offcanvas show={showOffcanvas} style={{zIndex: '9999'}} onHide={() => setShowOffcanvas(false)} placement="start">
            ...
            <Offcanvas.Body>
                ...
                <Form>
                  ...
                  <div className="text-center">
                    <Button
                      ...
                      className="d-flex justify-content-center align-items-center px-3 py-2 rounded mb-5 mt-3 mx-auto w-70"
                      style={{height: '35px'}}
                    >
                      {' '}
                  ...
                  <div className="text-center">
                    ...
                    {!showAutoGen && (
                      <Button
                        ...
                        className="d-flex justify-content-center align-items-center px-3 py-2 rounded mb-5 mt-1 mx-auto w-75"
                        style={{height: '35px', marginTop: '-4rem'}}
                      >
                        use Auto Generate
                      </Button>
                    )}
                  ...
                  <div className="offcanvas-footer p-2 position-absolute bottom-0 start-0 w-100">
                    <div className="d-flex align-items-start text-muted small ms-1">
                      ...
                      <span className="fst-italic" style={{fontSize: '12px', marginTop: '0.38rem'}}>
                        {' '}
                        How to use?{' '}
                      </span>
                    </div>
              ...
              <Modal show={showInstructions} onHide={() => setShowInstructions(false)} centered container={document.querySelector('.offcanvas-body')}>
                ...
                <Modal.Body>
                  <ol className="small mb-0 ps-3">
                    ...
                    <li>Click "Get Route" to generate path</li> {/* eslint-disable-next-line react/no-unescaped-entities */}
        ...
        <Offcanvas show={showDirectionsOffcanvas} onHide={handleOffcanvasClose} placement="end" className="custom-offcanvas" style={{maxHeight: 'fit-content', top: '3rem'}}>
          ...
          <Offcanvas.Body>
            {startLocation && !sortedChurches.length && (
              <div className="py-2 border-bottom" onClick={() => handleMarkerClick(startLocation)} style={{cursor: 'pointer'}}>
                <h6 className="mb-1 d-flex align-items-center">
                  <span className="badge rounded-circle bg-white d-flex align-items-center justify-content-center me-2" style={{width: '25px', height: '25px', background: 'linear-gradient(to right, #ff0000, #ff8000)'}}>
                    A
                  </span>
                  {churches.find(c => parseFloat(c.latitude) === startLocation.lat && parseFloat(c.longitude) === startLocation.lng)?.churchName || 'Custom Location'}
                </h6>
                <p className="ms-4 mb-0 text-muted">{churches.find(c => parseFloat(c.latitude) === startLocation.lat && parseFloat(c.longitude) === startLocation.lng)?.churchLocation || 'Custom Location'}</p>
              </div>
            )}

            {/* Destinations */}
            {uniqueDestinations.map((dest, index) => {
              const church = churches.find(c => parseFloat(c.latitude) === dest.lat && parseFloat(c.longitude) === dest.lng);
              ...
              return (
                <div key={index} className="py-2 border-bottom" onClick={() => handleMarkerClick(church)} style={{cursor: 'pointer'}}>
                  ...
                  <p className="ms-4 mb-0 text-muted">{church?.churchLocation || 'Custom Address'}</p>
                </div>
              );
            })}

            {sortedChurches.map((church, idx) => (
              <div key={idx} className="py-2 border-bottom" onClick={() => handleMarkerClick(church)} style={{cursor: 'pointer'}}>
                <h6 className="mb-1 d-flex align-items-center">
                  <span className="badge rounded-circle bg-white d-flex align-items-center justify-content-center me-2" style={{width: '25px', height: '25px', background: 'linear-gradient(to right, #ff0000, #ff8000)'}}>
                ...
                <p className="ms-4 mb-0 text-muted">{church.churchLocation || 'Location Not Available'}</p>
              </div>
            ))}
          </Offcanvas.Body>
        </Offcanvas>
